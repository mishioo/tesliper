...
class TestArrayProperty(ut.TestCase):
    ...
    def test_check_input_unmatching(self):
        arr = self.Clsx()
        arr.x = [1, 2]
        self.assertRaises(ValueError, self.Clsx.arr.check_input, arr, [1, 2, 3])
    ...
    def test_pad_more_sizes(self):
        ...
        self.assertSequenceEqual(out.tolist(), [[1, 2, 0], [3, 0, 0], [4, 5, 6]])
...
@mock.patch(
    "tesliper.glassware.array_base.ArrayBase.values", new_callable=mock.PropertyMock
)
@mock.patch(
    "tesliper.glassware.array_base.ArrayBase.filenames", new_callable=mock.PropertyMock
)
class TestArrayBase(ut.TestCase):
    ...
    def test_get_args(self, fnms, vals):
        ...
        arr = self.Sub(genre="bla", filenames=["f1", "f2", "f3"], values=[1, 2, 3])
    ...
    def test_get_args_not_stored_arg(self, fnms, vals):
        ...
        fnms.return_value = ["f1", "f2", "f3"]
        arr = Sub(
            genre="bla", filenames=["f1", "f2", "f3"], values=[1, 2, 3], other="foo"
        )
        self.assertDictEqual(
            arr.get_repr_args(),
            {
    ...
    def test_str(self, get, fnms, vals):
        ...
        fnms.return_value = m
        arr = self.Sub(genre="bla", filenames=["f1", "f2", "f3"], values=[1, 2, 3])
        self.assertEqual(str(arr), "[Sub of genre 'bla', 3 conformers]")
    ...
    def test_repr(self, get, fnms, vals):
        ...
        self.assertEqual(
            repr(arr),
            "Sub(genre='bla', filenames=['f1', 'f2', 'f3'], "
