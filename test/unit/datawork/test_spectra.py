...
class CountImaginaryTest(ut.TestCase):
    ...
    def test_empty_array(self):
        self.assertSequenceEqual(
            sp.count_imaginary(np.array([])).tolist(), []
        )

    def test_single_value_positive(self):
        self.assertEqual(
            sp.count_imaginary(np.array([1])).tolist(), 0
        )

    def test_single_value_zero(self):
        self.assertEqual(
            sp.count_imaginary(np.array([0])).tolist(), 0
        )

    def test_single_value_negative(self):
        self.assertEqual(
        sp.count_imaginary(np.array([-1])).tolist(), 1
        )

    def test_zero_dimensions_positive(self):
        self.assertEqual(
            sp.count_imaginary(np.array(1, dtype=int)).tolist(), 0
        )

    def test_zero_dimensions_zero(self):
        self.assertEqual(
            sp.count_imaginary(np.array(0, dtype=int)).tolist(), 0
        )

    def test_zero_dimensions_negative(self):
        self.assertEqual(
            sp.count_imaginary(np.array(-1, dtype=int)).tolist(), 1
            )
    ...
    def test_zero_imag_two_confs(self):
        self.assertSequenceEqual(
            sp.count_imaginary(np.arange(0, 6, 1).reshape(2, -1)).tolist(),
            [0, 0]
        )

    def test_one_imag_two_confs(self):
        self.assertSequenceEqual(
            sp.count_imaginary(np.arange(-1, 5, 1).reshape(2, -1)).tolist(),
            [1, 0]
        )

    def test_two_imag_two_confs(self):
        self.assertSequenceEqual(
            sp.count_imaginary(np.array([[1, -1, 1], [-1, 1, 1]])).tolist(),
            [1, 1]
        )

    def test_tree_dimensions(self):
        self.assertRaises(
            ValueError, sp.count_imaginary, np.array([[[1]]])
        )
...
class FindImaginaryTest(ut.TestCase):
    ...
    def test_more_dimensions(self):
        self.assertRaises(
            ValueError, sp.find_imaginary, np.array([[[1]]])
        )
...
class GaussianTest(ut.TestCase):
    ...
    def test_mean_0_std_dev_1(self):
        np.testing.assert_array_almost_equal(
            sp.gaussian(
                np.array([1]), np.array([0]), np.arange(-5, 6, 1), np.sqrt(2)
            ),
            [
    ...
    def test_empty_intensities(self):
        out = sp.gaussian(
            np.array([]), np.array([]), np.arange(-5, 6, 1), 1
        )
...
class CalculateSpectraTest(ut.TestCase):
    ...
    def test_unmatching_arrays(self):
        self.assertRaises(
            ValueError, sp.calculate_spectra,
            np.ones((2, 2)), np.ones((3, 2)), np.arange(3), 1, self.fitting
        )
